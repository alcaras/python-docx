...
# ugly globals but shit works
header_to_insert = deque([])
footer_to_insert = deque([])
...
#return the actual header and the break
def new_header(hdr_content, relationships):
    # add the header to the relationships
    relationships += [new_header_relationship()]

    # get the relationship number of the header
    hdr_num = len(relationships)

    # create the actual header
    hdr1 = newheader(hdr_content)

    # get what needs to be inserted into the next break
    # for the header to work

    hdr_break = new_header_break(hdr_num)

    global list_of_headers
    list_of_headers += [hdr1]
#    print list_of_headers
    
    global header_to_insert
    header_to_insert.append(hdr_break)

def newheader(paragraph):
    header = makeelement('hdr')
    header.append(paragraph)
    return header

def new_header_relationship():
    global header_count, header_rel
    header_count += 1
#    print "incremented header_count"
    new_rel = [header_rel, 'header'+str(header_count)+'.xml']
    return new_rel

def new_header_break(hdr_num):
    hdrRef = makeelement('headerReference',
...
#return the actual footer and the break
def new_footer(ftr_content, relationships):
    # add the footer to the relationships
    relationships += [new_footer_relationship()]

    # get the relationship number of the footer
    ftr_num = len(relationships)

    # create the actual footer
    ftr1 = newfooter(ftr_content)

    # get what needs to be inserted into the next break
    # for the footer to work

    ftr_break = new_footer_break(ftr_num)

    global list_of_footers
    list_of_footers += [ftr1]
#    print list_of_footers
    
    global footer_to_insert
    footer_to_insert.append(ftr_break)

def newfooter(paragraph):
    footer = makeelement('ftr')
    footer.append(paragraph)
    return footer

def new_footer_relationship():
    global footer_count, footer_rel
    footer_count += 1
#    print "incremented footer_count"
    new_rel = [footer_rel, 'footer'+str(footer_count)+'.xml']
    return new_rel

def new_footer_break(ftr_num):
    ftrRef = makeelement('footerReference',
                          attributes={'type':'default',
                                      'id':'rId'+str(ftr_num)},
                          apa=['w','r'])
    return ftrRef
...
def makeelement(tagname, tagtext=None, nsprefix='w', attributes=None, attrnsprefix=None, apa=None, preserve=False):
    # apa = for other namespaces
    # preserve = ugly hack for xml:space="preserve"
    ...
    if attributes:
        ...
        for i, tagattribute in enumerate(attributes):
            if not apa:
                newelement.set(attributenamespace+tagattribute, attributes[tagattribute])
            else:
                attributenamespace = '{' + nsprefixes[apa[i]] + '}'
                newelement.set(attributenamespace+tagattribute, attributes[tagattribute])



    if preserve == True:
        newelement.set('{http://www.w3.org/XML/1998/namespace}space' ,"preserve")
#        print "TROO", etree.tostring(newelement)

    if tagtext:
        newelement.text = tagtext
    return newelement

def landscape(orient='landscape'):
    # make the final section a landscape
    sectPr = makeelement('sectPr')

    # point to headers and footers here
    ...
    # 7 8  header even def
    # 9 10 footer even def
    # 11   header first
    # 12   footer first
    
    # hdrRefA = makeelement('headerReference',
    #                       attributes={'type':'default',
    #                                   'id':'rId7'},
    #                       apa=['w','r'])
    # # hdrRefB = makeelement('headerReference',
    # #                       attributes={'type':'default',
    # #                                   'id':'rId8'},
    # #                       apa=['w','r'])
    # # hdrRefC = makeelement('headerReference',
    # #                       attributes={'type':'even',
    # #                                   'id':'rId11'},
    # #                       apa=['w','r'])
    
    
    # # ftrRefA = makeelement('footerReference',
    # #                       attributes={'type':'even',
    # #                                   'id':'rId9'},
    # #                       apa=['w','r'])
    # ftrRefA = makeelement('footerReference',
    ...
    #                                   'id':'rId8'},
    #                       apa=['w','r'])
    # # ftrRefC = makeelement('footerReference',
    #                       attributes={'type':'even',
    #                                   'id':'rId12'},
    #                       apa=['w','r'])
    ...
    global header_to_insert
    try:
        hdrRef = header_to_insert.popleft()
    except IndexError:
        hdrRef = None
    
    if hdrRef is not None:
#        print "appended a header"
        sectPr.append(hdrRef)

    global footer_to_insert
    try:
        ftrRef = footer_to_insert.popleft()
    except IndexError:
        ftrRef = None
    
    if ftrRef is not None:
#        print "appended a footer"
        sectPr.append(ftrRef)

    # sectPr.append(hdrRefB)
    # sectPr.append(hdrRefC)

#    if ftrRef is not None:
#        print "appended a footer"
#        sectPr.append(ftrRef)
    # sectPr.append(ftrRefB)
    # sectPr.append(ftrRefC)
...
def pagebreak(type='page', orient='portrait', valign='top'):
...
#            print "appended a footer"
        ...
        # http://www.schemacentral.com/sc/ooxml/t-w_ST_VerticalJc.html
        if valign != 'top':
            vAlign = makeelement('vAlign', attributes={'val':valign})
...
def paragraph(paratext, style='BodyText', breakbefore=False, jc='left'):
    ...
    in paratext, [1] is string containing a combination of 'buirPp$' chars
        ...
        for pt in paratext:
            if isinstance(pt, (list, tuple)):
                ...
                if pt[1].find('$') > -1:
                    pf = True
#                    print "TROO", pf, pt[0]
                text.append([makeelement('t', tagtext=pt[0], preserve=pf), pt[1]])
            else:
    ...
    for t in text:
        ...
        if t[1].find('r') > -1:
            ...
            run.append(br)
        if t[1].find('p') > -1:
            # add in the page number
            field = makeelement('fldSimple', attributes={'instr':'PAGE \\* MERGEFORMAT'})
            run.append(field)
            pass
        if t[1].find('P') > -1:
            # add in the total number of pages
            # don't need this: \\# "0" \\* Arabic
            field = makeelement('fldSimple', attributes={'instr':'NUMPAGES \\* MERGEFORMAT'})
            run.append(field)
            pass
        run.append(rPr)
        ...
        run.append(t[0])
#        pprint(etree.tostring(run))
        paragraph.append(run)
    # Return the combined paragraph
    return paragraph

def contenttypes(useHF=False):
    ...
    for i in range(1, header_count+1):
        parts.update({'/word/header'+str(i)+'.xml' : header_type})
        
    for i in range(1, footer_count+1):
        parts.update({'/word/footer'+str(i)+'.xml' : footer_type})

        # parts.update({'/word/header1.xml' : header_type})
        # parts.update({'/word/header2.xml' : header_type})
        # parts.update({'/word/header3.xml' : header_type})

        # parts.update({'/word/footer1.xml' : footer_type})
        # parts.update({'/word/footer2.xml' : footer_type})
        # parts.update({'/word/footer3.xml' : footer_type})
...
def picture(relationshiplist, fullpath, picname, picdescription, pixelwidth=None, pixelheight=None, nochangeaspect=True, nochangearrowheads=True, wrapInParagraph=True):
    ...
    if not os.path.isdir(media_dir):
        os.mkdir(media_dir)
    shutil.copyfile(fullpath, join(media_dir, picname))
    
    # Check if the user has specified a size
    if not pixelwidth or not pixelheight:
        # If not, get info from the picture itself
        pixelwidth, pixelheight = Image.open(fullpath).size[0:2]
    ...
    # scale by 3/4 (This may be because I resaved at 96dpi)
    # we can also enforce a max height and max width
    width = int(pixelwidth * emuperpixel * 3/4)
    height = int(pixelheight * emuperpixel * 3/4)

    # @@ TODO enforce max height and max width
    # maxwidth = 16 cm
    # maxheight = 16 cm

    maxheight = maxwidth = 5900000

#    print width, maxwidth, picname

    scale = float(width)/float(height)
    if width > maxwidth:
        print "rescaling width", picname, width, maxwidth
        width = maxwidth
        height = int(float(width)/ float(scale))
    
    if height > maxheight:
        print "rescaling height", picname, height, maxheight
        height = maxheight
    ...
    run = makeelement('r')
    run.append(drawing)
    if wrapInParagraph == True:
        paragraph = makeelement('p')
        paragraph.append(run)
        return relationshiplist, paragraph
    else:
        return relationshiplist, run
...
def relationshiplist():
    relationshiplist =\
        ...
        , [ 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme'       , 'theme/theme1.xml' ]
          ...
          # gonna need multiples... 1 set of 3 per section, i bet
#        , [ header_rel, 'header1.xml' ]
        # , [ header_rel, 'document/header2.xml' ]
#        , [ footer_rel, 'footer1.xml' ]
        # , [ footer_rel, 'document/footer2.xml' ]
        # , [ header_rel, 'document/header3.xml' ]
        # , [ footer_rel, 'document/footer3.xml' ]
        ]
...
def wordrelationships(relationshiplist):
    ...
    relationships = etree.fromstring('<Relationships xmlns="http://schemas.op'
        'enxmlformats.org/package/2006/relationships"></Relationships>')
    count = 0
...
def savedocx(document, coreprops, appprops, contenttypes, websettings, wordrelationships, output):
    ...
    for i, hdr in enumerate(list_of_headers):
        treesandfiles.update({hdr : 'word/header' + str(i+1) + '.xml'})

    for i, ftr in enumerate(list_of_footers):
        treesandfiles.update({ftr : 'word/footer' + str(i+1) + '.xml'})
